...
class Servicio(EntidadSistema):
    ...
    def validar_duracion(self, duracion_horas):
        if not isinstance(duracion_horas, int) or duracion_horas < 1:
            raise ErrorDuracionInvalida(f'Duracion debe ser entero positivo. Recibido: {duracion_horas}')
        if duracion_horas < self._duracion_min or duracion_horas > self._duracion_max:
            raise ErrorDuracionInvalida(f'Duracion {duracion_horas}h fuera del rango [{self._duracion_min}h-{self._duracion_max}h]')
        return True
...
class ReservaSala(Servicio):
    ...
    def calcular_costo(self, duracion_horas):
        try:
            ...
            raise ErrorCalculoCosto('Error en costo de sala') from ex
    ...
    def calcular_costo_con_descuento(self, duracion_horas, porcentaje_descuento):
        if not (0.0 <= porcentaje_descuento <= 1.0):
            raise ErrorCalculoCosto('Descuento invalido. Use entre 0.0 y 1.0')
    ...
    def describir(self):
        ...
        return (f'[SALA] {self._nombre} | Capacidad: {self._capacidad} personas | Precio: \/h | Rango: {self._duracion_min}h-{self._duracion_max}h | {estado}')
...
class AlquilerEquipo(Servicio):
    ...
    def calcular_costo(self, duracion_horas):
        try:
            ...
            raise ErrorCalculoCosto('Error en costo de equipo') from ex
    def calcular_costo_con_impuesto(self, duracion_horas, tasa_impuesto=None):
        tasa = tasa_impuesto if tasa_impuesto is not None else self.IMPUESTO_DEFAULT
        costo = self.calcular_costo(duracion_horas)
    ...
    def describir(self):
        estado = 'Disponible' if self._disponible else 'No disponible'
        return (f'[EQUIPO] {self._nombre} ({self._tipo_equipo}) | Precio: \/h | Rango: {self._duracion_min}h-{self._duracion_max}h | Deposito: 20% | {estado}')
...
class AsesoriaEspecializada(Servicio):
    ...
    def __init__(self, nombre, precio_base, area, nivel_asesor='junior', cargo_sesion=0):
        ...
        if nivel not in self._MULTIPLICADORES:
            raise ErrorServicioNoDisponible(f'Nivel invalido. Use: {list(self._MULTIPLICADORES.keys())}')
    ...
    def calcular_costo(self, duracion_horas):
        try:
            ...
            raise ErrorCalculoCosto('Error en costo de asesoria') from ex
    def calcular_costo_con_impuesto(self, duracion_horas, tasa_impuesto=None):
        tasa = tasa_impuesto if tasa_impuesto is not None else self.IMPUESTO_DEFAULT
        return round(self.calcular_costo(duracion_horas) * (1 + tasa), 2)
    def calcular_costo_con_descuento(self, duracion_horas, porcentaje_descuento):
    ...
    def describir(self):
        estado = 'Disponible' if self._disponible else 'No disponible'
        mult = self._MULTIPLICADORES[self._nivel_asesor]
        return (f'[ASESORIA] {self._nombre} | Area: {self._area} | Nivel: {self._nivel_asesor.capitalize()} (x{mult}) | Precio: \/h | Cargo sesion: \ | {estado}')