...
class Reserva(EntidadSistema):
    ...
    def __init__(self, cliente, servicio, duracion_horas, notas=""):
        ...
        if not cliente.activo:
            raise ErrorReservaInvalida("El cliente esta inactivo.")
        if not servicio.disponible:
            raise ErrorServicioNoDisponible("El servicio no esta disponible.")
    ...
    def confirmar(self, aplicar_impuesto=True, descuento=0.0):
        try:
            ...
            log.error(f"CONFIRMAR RESERVA FALLIDA | ID: {self._id} | Error: {ex}")
            raise
        except Exception as ex:
            log.critical(f"ERROR INESPERADO confirmando [{self._id}]: {ex}")
            raise ErrorCalculoCosto("Error inesperado") from ex
        else:
            ...
            log.info(
                ...
                f"Duracion: {self._duracion_horas}h | "
                f"Costo: ${self._costo_final:,.2f}")
            return self._costo_final
        finally:
            log.debug(
                f"Intento confirmacion | Reserva: {self._id} | Estado: {self._estado}")
    ...
    def completar(self):
        if self._estado != "confirmada":
            raise ErrorOperacionNoPermitida(
                f"Solo se completan reservas confirmadas. Estado: {self._estado}")
